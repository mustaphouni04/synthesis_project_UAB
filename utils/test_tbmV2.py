...
def test(model, test_loader, device="cuda", save=True):
    ...
    if save:
        ...
        with open("reconstructionsV2.txt", "w") as f:
        ...
        dummy_input = torch.randn(1, 20, 112).to(device)  # Ensure dummy input is on the same device
        torch.onnx.export(model,  # model being run
                          dummy_input,  # model input (or a tuple for multiple inputs)
                          "modelV2.onnx",  # where to save the model (can be a file or file-like object)
